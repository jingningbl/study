...
window.onload=function(){
    ...
    $usernameObj.blur(function (){
        ...
        let $span=$("#err-username");
        if (!reg.test($username)){
            flag_username=false;
            $span.text("格式错误!3-8数字字母首字符为字母").css({"color":"red"});
        }else {
            $.ajax({
                url: "findByUsername",
                type: "get",
                ...
                success: function (msg) {
                    if (msg) {
                        flag_username=true;
                        $span.text("可以使用").css({"color": "green"});
                    } else {
                        flag_username=false;
                        $span.text("无法使用").css({"color": "red"});
                    }
                }
            });
        }
    ...
    $pwdObj.blur(function (){
        ...
        if (reg.test($pwdObj.val())){
            flag_pwd=true;
            $span.text("输入合法").css({"color":"green"});
        }else {
            flag_pwd=false;
            $span.text("格式错误").css({"color":"red"});
        }
    });
    // //密码验证
    ...
    $rePwd.blur(function (){
        let $span=$("#err-rePwd");
        if ($pwdObj.val()===$rePwd.val()){
            flag_repwd=true;
            $span.text("密码一致").css({"color":"green"});
        }else {
            flag_repwd=false;
            $span.text("密码不一致").css({"color":"red"});
        }
    });
    //form表单的onsubmit事件
    let myform=document.getElementsByTagName("form")[0];
    myform.onsubmit=function(){
        return flag_username && flag_pwd && flag_repwd;
    };
