...
function sumGoods() {
  ...
  for (let i = 1; i < trs.length - 1; i++) {
    ...
    //转数字
    let price = Number.parseInt($(xjObj).html());
    //总和
    sum += price;
  }
  //显示结果
...
function reduce(obj) {
  //获取数值
  let $numObj = $(obj).next();
  let num = Number.parseInt($numObj.val());
  //数量>=1
  if (num >= 1) {
    //数量-1
    $numObj.val(num - 1);
    //小计
    let $xjObj = $(obj).parent().next();
    //单价
    let $priceObj = $(obj).parent().prev();
    //单价转数字
    let price = Number.parseInt($priceObj.html());
    //计算小计
    $xjObj.html(price * (num - 1));
  } else {
    //数量<1,即=0,禁用按钮
    $(obj).disabled = true;
  }
...
function dete(obj) {
  //获取表格
  let $ta = $(obj).parent().parent().parent();
  //获取行并删除
...
function addGood(obj) {
  ...
  let trs = $ta.getElementsByTagName("tr");
  //获取添加的信息
  let $tr = $(obj).parent().parent().get(0);
  //克隆信息
